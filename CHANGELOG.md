...
- Changes to one subscriber now run under a per-domain Redis lock shared by every replica: an inbox Follow, Undo, or Reject and an admin accept, reject, remove, or block can no longer interleave between reading the record and writing it, so an admin accept is no longer overwritten by a concurrent re-Follow, concurrent LitePub Follows no longer send outbound Follows with ids the relay does not keep, and an Undo no longer removes the subscription a concurrent re-Follow just created. Every lock acquisition gets an increasing per-domain sequence and subscriber writes are fenced by it, so a request whose lock expired while it stalled cannot overwrite a change written by a later holder, while a late write nobody overtook still commits. An inbox request that waits more than 5 seconds for the lock, cannot take it, or is overtaken before writing gets 503 with `Retry-After` and releases its dedup slot so the retry is processed; the admin API answers 409 for a wait timeout or an overtaken write
- Accept, Reject, Follow, and Undo Follow notifications that follow from a subscriber change are now recorded in a Redis outbox by the same script that writes the subscriber, and delivered from there: a change refused because the domain was blocked or a later request overtook it sends no notifications, and a notification that fails to queue after its change committed is retried by a job that runs every minute instead of being lost. Before sending, each notification job checks that the subscriber is still in the state it announces and skips one a later change made obsolete (e.g. a Follow recovered after the subscriber was removed), and Accept, Reject, and Undo Follow keep the activity id chosen when they were recorded, so retries and redeliveries send the same activity instead of a new one each time
